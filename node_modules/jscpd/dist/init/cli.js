...
function initCli(packageJson, argv) {
    const cli = new commander_1.Command(packageJson.name);
    cli.version(packageJson.version)
        .usage('[options] <path ...>')
        .description(packageJson.description)
        .option('-l, --min-lines [number]', 'min size of duplication in code lines (Default is ' + core_1.getOption('minLines') + ')')
        .option('-k, --min-tokens [number]', 'min size of duplication in code tokens (Default is ' + core_1.getOption('minTokens') + ')')
        .option('-x, --max-lines [number]', 'max size of source in lines (Default is ' + core_1.getOption('maxLines') + ')')
        .option('-z, --max-size [string]', 'max size of source in bytes, examples: 1kb, 1mb, 120kb (Default is ' + core_1.getOption('maxSize') + ')')
        .option('-t, --threshold [number]', 'threshold for duplication, in case duplications >= threshold jscpd will exit with error')
        ...
        .option('-r, --reporters [string]', 'reporters or list of reporters separated with comma to use (Default is time,console)')
        .option('-o, --output [string]', 'reporters to use (Default is ./report/)')
        .option('-m, --mode [string]', 'mode of quality of search, can be "strict", "mild" and "weak" (Default is "' + core_1.getOption('mode') + '")')
